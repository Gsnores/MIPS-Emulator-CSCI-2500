...
void iplc_sim_push_pipeline_stage()
{
    ...
    if (pipeline[MEM].itype == LW) {
		...
		int moveTo=pipeline[MEM].stage.lw.dest_reg; //where the register is moving to
	    //this long if statement checks whether or not there will be a data hazard, which would cause an LW delay
			if ( (pipeline[ALU].itype==RTYPE &&  
				(pipeline[ALU].stage.rtype.reg1==moveTo || pipeline[ALU].stage.rtype.reg2_or_constant==moveTo || pipeline[ALU].stage.rtype.dest_reg==moveTo)) || 
				(pipeline[ALU].itype==LW && 
					pipeline[ALU].stage.lw.dest_reg==moveTo || pipeline[ALU].stage.lw.base_reg == moveTo) ||
				(pipeline[ALU].itype==SW && 
					(pipeline[ALU].stage.sw.base_reg==moveTo || pipeline[ALU].stage.sw.src_reg==moveTo)) ||
				(pipeline[ALU].itype==BRANCH && 
					(pipeline[ALU].stage.branch.reg1 == moveTo || pipeline[ALU].stage.branch.reg2 == moveTo)) )
					pipeline_cycles++; //if there's a data hazard/LW delay, increase pipeline cycles
			}
		if (pipeline[ALU].itype == RTYPE)
		{
		...
		if (!hit)
		{
			inserted_nop += 10;//delay for missing cache
		}
		pipeline_cycles+=inserted_nop; //increment pipeline cycles based on misses and data hazards
    }
    ...
    if (pipeline[MEM].itype == SW) {
		...
		if (!data_hit)
			pipeline_cycles+=CACHE_MISS_DELAY; //if cache missed, there is a delay in the pipeline
    }
