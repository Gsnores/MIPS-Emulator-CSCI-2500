...
void iplc_sim_init(int index, int blocksize, int assoc)
{
	...
	for (i = 0; i < (1<<index); i++) {
		cache_line_t a;
		a.valid_bit = (int*) malloc(cache_assoc * sizeof(int));
		a.tag = (int*) malloc(cache_assoc * sizeof(int));
		a.LRU = (long int*) malloc(cache_assoc * sizeof(long int));
...
void iplc_sim_push_pipeline_stage()
{
	...
	if (pipeline[MEM].itype == SW) {
		int delay_cycles = 0;
		int address_needed = pipeline[MEM].stage.sw.base_reg;
		int address_storing_into = pipeline[MEM].stage.sw.src_reg;
		if (pipeline[ALU].itype == RTYPE)
		{
			int address_used = pipeline[ALU].stage.rtype.dest_reg;
			if (address_needed == address_used)
		...
		if (iplc_sim_trap_address(address_storing_into) == 0) //store caused a cache miss
		{
			delay_cycles += 10; 
		}
		pipeline_cycles += delay_cycles; 
	}
